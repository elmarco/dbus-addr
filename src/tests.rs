...
#[test]
fn parse_other() {
    const ADDR: &str = "foo:arg=val";

    let addr = DBusAddr::try_from(ADDR).unwrap();
    let transport = addr.transport().unwrap();
    assert!(matches!(transport, Transport::Other(Cow::Borrowed("foo"))));
